...
9. **`LSC` is authentic but truncated, and truncated with a slant** — the 66 values present match the published Legatum 2023 social capital pillar on every spot-check, so this is a real import that stopped short, not an invention. Legatum publishes all 167 countries. Splitting this index's countries into quartiles by `cc`, the number carrying an `LSC` value runs **34 / 22 / 10 / 0** from best-governed to worst, so an 8%-weight component is missing precisely where it would most likely score low, and proportional reweighting hands that weight to components the country does have. Modelling the missing values from each country's other components moves 72 of 173 ranks, median 1 place, max 7 (Belize +7) — the fix is a correctness and honesty matter more than a reshuffle. Legatum's domains are blocked from the build sandbox, so completing it needs a supplied export.
