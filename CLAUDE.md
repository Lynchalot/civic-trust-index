...
6. **Coverage exceeds source scope in `WVS` and `GLO`** — but by less than first recorded here. `WVS` holds 107. Wave 7 itself ran 64–66 surveys, but the standard combined release is the **Joint EVS/WVS 2017–2022 dataset, 92 countries and territories** (36 from EVS 2017, 66 from WVS7, 10 countries in both), which is the defensible universe for a European-inclusive trust series — so the surplus is on the order of 15, not the 43 an earlier note here implied. `GLO` holds 170 against the **141 countries and territories** Gallup's own 2023 Global Law and Order report says it surveyed — a surplus of about 29. Neither dict carries a source comment, unlike `SE`, so where the extras came from is unrecorded. Pinning down exactly which entries are surplus needs the two published country lists; both survey sites are blocked by the sandbox egress policy, and web search returns conflicting counts, so this has to come from a supplied list.
7. **Micro-state values in `GLO` and `EPI` look invented, and one fabricated entry is still in the file** — Liechtenstein, Monaco, San Marino, Andorra and Vatican City appear in exactly three dictionaries — `GLO`, `HOM`, `EPI` — and nowhere else. In two of those the values form neat descending runs matching a prestige order rather than measurements: `GLO` 95/92/90/89/88 and `EPI` 92/90/88/88/85. The clearest tell is Vatican City carrying a Gallup Law & Order score of 95, for a state of ~800 residents that no pollster surveys. **`GLO` still holds that VAT:95 entry** — it stopped affecting the index only because the Holy See has no WGI record and so is not scored, not because the value was removed. `HOM` is plausible — UNODC does publish homicide counts for these states. Against that, the top of `GLO` is not junk: Tajikistan at 96 is a real and repeatedly published Gallup result, so this reads as a genuine core with fabricated fill-ins rather than a fabricated dictionary.
8. **The top four ranks rest on 5 of 13 components, and pruning cannot fix it** — Liechtenstein, Monaco, San Marino and Andorra hold ranks 1, 2, 4 and 5 scored on `cc`, `ge`, `glo`, `hom`, `epi` only (31% of the total weight). They never face the wallet test, the bribery rate, interpersonal trust, integrity of communities, road deaths, the shadow economy, terrorism or infrastructure — and the 2026 Legatum edition does not cover them either, so that gap is not closing. Proportional reweighting collapses the full 100% onto the five that remain, all of which favour small, rich, stable places. This has now been measured: deleting the suspect `GLO` and `EPI` values from all four leaves them on **three** components and makes **Monaco 1st**, ahead of Liechtenstein. Removing bad data makes the problem worse, because reweighting rewards absence. Dimming them in the rankings does not touch it either. The fix has to be a rule about coverage — excluding countries below a weight-coverage threshold from the headline ranking, or applying an explicit confidence penalty. At a 50% weight-coverage floor, 25 of 173 fall out of the ranking and the index opens Singapore, Denmark, Norway, Mauritius, Sweden.
9. **`LSC` changed source and meaning in the 2026 import** — the slot held Legatum's Social Capital pillar (2023): real values, verified against the published index, but only 66 countries, and skewed — by quartile of `cc` the coverage ran **34 / 22 / 10 / 0** from best-governed to worst, so an 8% weight vanished exactly where it would most likely have scored low. The 2026 edition rebuilt the index into three domains and ten pillars and dropped Social Capital entirely. The slot now holds **Integrity of Communities** (2026 data sheet, `scores` tab), 160 of 173 countries. It is the closest published successor, not the same measure: across the 66 countries holding both, the two correlate at only **r = 0.51**, so the component was renamed throughout rather than quietly refilled. The swap moved 141 of 173 ranks, median 3, max 43 — the largest falls (Eritrea −43, Turkmenistan −41, Equatorial Guinea −35) are countries that had been gaining from the pillar's absence. The 13 still missing (Liechtenstein, Monaco, San Marino, Andorra, Brunei, Cuba, Guyana, Hong Kong, Maldives, Syria, Taiwan, Ukraine, Kosovo) fall outside the 2026 index. **Its vintage, 2026, is now the newest in the index** — see item 5 on the widening spread.
10. **Digit profiling of the remaining unverified dicts** — `SE`, `GCB_RAW` and `INF_RAW` have last-digit distributions consistent with real measurement (χ² 9.6, 8.7, 7.7 against uniform, df 9, 5% critical value 16.9); nothing there looks fabricated. `HOM` (χ² 202) and `RTR` (χ² 62) cluster hard on `.0` and `.5` — 61% and 43% of values sit on a half-unit grid. That is what rounding a published rate to one decimal looks like, so it is not by itself evidence of invention, but neither dict has been checked against UNODC or WHO.
11. **No bundler or minification** — JS and CSS are served as raw source files.
...
- **Verify the remaining source values** — `WLT` (Cohn et al. 2019), `GTI` (IEP 2026), `WGI`/`GE` (World Bank 2024) and `LSC` (Legatum 2026) have been checked against their primary sources; `WGI`/`GE`, `GTI` and `LSC` were all re-imported as a result. Structural profiling has flagged `WVS`, `GLO` and the micro-state entries in `EPI` (items 6–8), and cleared `SE`, `GCB_RAW` and `INF_RAW` on digit distribution alone (item 10). That leaves `SE`, `GCB_RAW`, `HOM`, `RTR` and `INF_RAW` unchecked against their actual sources. The other twelve components' figures came from earlier imports of varying provenance; the WGI case above shows those imports are not automatically trustworthy. Coverage counts and ranges quoted in Section 04 are computed from `js/data.js` itself and are accurate to the file, but that is not the same as being accurate to the source.
