...
5. **GTI values are not the published index** — the `GTI` dict in `js/data.js` does not match IEP's Global Terrorism Index 2023. The published index covers 163 countries with **70 scoring exactly 0.000** and a maximum of 8.822 (Afghanistan); this file has 116 countries, **none scoring 0**, and a maximum of 9.1. Countries the published index scores zero carry invented values here (Panama 2, Kuwait 1.5, Singapore 1, El Salvador 4.5), and the last decimal clusters implausibly (32 values end in `.5`, one ends in `.4`). Because `nGTI = 100 − score×10`, countries that should score 0 are penalised 10–45 points on an 8%-weighted component. Correcting only the four verifiable cases moves 47 of 173 ranks (El Salvador +30 places, +5.00 points); the full correction covers ~70 countries and is larger. This is the most consequential known data problem in the project. Fix by re-importing GTI 2023 scores from visionofhumanity.org / economicsandpeace.org — both blocked by the sandbox egress policy, so it needs an environment with access.
